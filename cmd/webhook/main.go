...
var deploymentTypes = map[schema.GroupVersionKind]resourcesemantics.GenericCRD{
	// List the types to validate.
...
func NewDeploymentValidationAdmissionController(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
	return validation.NewAdmissionController(ctx,

		// Name of the resource webhook.
		fmt.Sprintf("validation.webhook.deployment.%s.knative.dev", system.Namespace()),
...
func main() {
	...
	sharedmain.WebhookMainWithContext(ctx, "webhook",
		...
		NewDeploymentValidationAdmissionController,
	)
}
